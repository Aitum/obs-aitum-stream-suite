...
OBSProjector::OBSProjector(obs_canvas_t *canvas_, obs_source_t *source_, int monitor,
			   std::function<void(OBSProjector *projector)> deleteProjector_)
	: OBSQTDisplay(nullptr, Qt::Window),
	  canvas(canvas_),
	  source(source_),
	  deleteProjector(deleteProjector_)
{
	if (source) {
		sigs.emplace_back(obs_source_get_signal_handler(source), "rename", OBSSourceRenamed, this);
		sigs.emplace_back(obs_source_get_signal_handler(source), "remove", OBSSourceDestroyed, this);
		sigs.emplace_back(obs_source_get_signal_handler(source), "destroy", OBSSourceDestroyed, this);
	}

	isAlwaysOnTop = config_get_bool(obs_frontend_get_user_config(), "BasicWindow", "ProjectorAlwaysOnTop");
	...
	if (canvas) {
		...
		UpdateProjectorTitle(QString::fromUtf8(obs_source_get_name(source)));
	}
...
void OBSProjector::SetHideCursor()
{
	...
	bool hideCursor = config_get_bool(obs_frontend_get_user_config(), "BasicWindow", "HideProjectorCursor");
...
void OBSProjector::OBSRender(void *data, uint32_t cx, uint32_t cy)
{
	...
	obs_canvas_t *canvas = window->canvas;
	obs_source_t *source = window->source;
	...
	if (canvas && obs_canvas_get_video_info(canvas, &ovi)) {
		...
		targetCX = obs_source_get_width(source);
		targetCY = obs_source_get_height(source);
	}
...
void OBSProjector::mousePressEvent(QMouseEvent *event)
{
	...
	if (event->button() == Qt::RightButton) {
		...
		CanvasDock::AddProjectorMenuMonitors(projectorMenu, this, SLOT(OpenFullScreenProjector()));
...
{
	if (deleteProjector)
		deleteProjector(this);
}

void OBSProjector::UpdateProjectorTitle(QString name)
{

	QString title = QString::fromUtf8(obs_frontend_get_locale_string("Projector.Title")) + " - ";
	if (canvas) {
		title += QString::fromUtf8(obs_canvas_get_name(canvas));
	} else if (source) {
		if (obs_source_is_scene(source)) {
			title += QString::fromUtf8(obs_frontend_get_locale_string("Projector.Title.Scene")).arg(name);
		} else {
			title += QString::fromUtf8(obs_frontend_get_locale_string("Projector.Title.Source")).arg(name);
		}
	}
