...
#endif
{
	...
	std::lock_guard<std::mutex> l(*tree->selectMutex);
	tree->hoveredPreviewItems->clear();
	tree->hoveredPreviewItems->push_back(sceneitem);
}
...
void SourceTreeModel::AddGroup()
{
	...
	obs_sceneitem_t *group = obs_scene_add_group(st->getScene(st->getSceneParam), name.toUtf8().constData());
	if (!group)
		return;
...
void SourceTree::dropEvent(QDropEvent *event)
{
	...
	obs_scene_t *scene = getScene(getSceneParam);
	SourceTreeModel *stm = GetStm();
